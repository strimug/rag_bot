"""
Telegram-бот с RAG-функциональностью на основе OpenRouter API.

Этот бот использует HTTP запросы к OpenRouter API, что позволяет
работать с различными моделями LLM через единый интерфейс.
"""
...
async def cmd_start(message: Message):
    ...
    welcome_text = """
🤖 <b>Добро пожаловать в RAG-бота (OpenRouter)!</b>

Я интеллектуальный ассистент с доступом к базе знаний.
Работаю через OpenRouter API - единый интерфейс для различных моделей LLM.
...
/test - Проверить подключение к OpenRouter API
...
async def cmd_help(message: Message):
    ...
    help_text = """
📖 <b>Подробная справка по использованию бота (OpenRouter версия)</b>
...
🔹 API: OpenRouter (OpenAI-совместимый)
🔹 Модель чата: openai/gpt-4o-mini
🔹 Модель vision: openai/gpt-4o-mini
🔹 Эмбеддинги: openai/text-embedding-3-small
🔹 Векторная БД: FAISS
🔹 HTTP клиент: requests

<b>Преимущества OpenRouter версии:</b>
✅ Доступ к различным моделям LLM через единый интерфейс
...
async def cmd_ingest(message: Message):
    ...
    await message.answer("📥 Начинаю индексацию документов через OpenRouter...")
...
async def cmd_stats(message: Message):
    ...
    stats_text = f"""
📊 <b>Статистика RAG-системы (OpenRouter)</b>
...
async def cmd_test(message: Message):
    """
    Обработчик команды /test - проверка подключения к OpenRouter API.
    """
    logger.info(f"Проверка подключения от пользователя {message.from_user.id}")
    
    await message.answer("🔄 Проверяю подключение к OpenRouter API...")
    
    try:
        success = rag_pipeline.test_connection()
        
        if success:
            await message.answer("✅ Подключение к OpenRouter API работает корректно!")
        else:
            await message.answer("❌ Проблемы с подключением к OpenRouter API. См. логи.")
...
async def handle_photo(message: Message):
    ...
    processing_msg = await message.answer("🖼 Обрабатываю изображение через OpenRouter...")
...
async def main():
    ...
    """
    logger.info("=" * 50)
    logger.info("Запуск Telegram-бота с RAG (OpenRouter)")
    logger.info("=" * 50)
