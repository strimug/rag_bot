"""
Telegram-бот с RAG-функциональностью на основе ProxyAPI.
...
async def cmd_start(message: Message):
    ...
    welcome_text = """
🤖 <b>Добро пожаловать в RAG-бота (ProxyAPI)!</b>
...
async def cmd_help(message: Message):
    ...
    help_text = """
📖 <b>Подробная справка по использованию бота (ProxyAPI версия)</b>
...
<b>Преимущества ProxyAPI версии:</b>
...
async def cmd_stats(message: Message):
    ...
    stats_text = f"""
📊 <b>Статистика RAG-системы (ProxyAPI)</b>
...
async def cmd_test(message: Message):
    ...
    await message.answer("🔄 Проверяю подключение к ProxyAPI...")
    
    try:
        success = rag_pipeline.test_connection()
        
        if success:
            await message.answer("✅ Подключение к ProxyAPI работает корректно!")
        else:
            await message.answer("❌ Проблемы с подключением к ProxyAPI. См. логи.")
...
async def handle_photo(message: Message):
    ...
    processing_msg = await message.answer("🖼 Обрабатываю изображение через ProxyAPI...")
...
async def main():
    ...
    """
    logger.info("=" * 50)
    logger.info("Запуск Telegram-бота с RAG (ProxyAPI)")
    logger.info("=" * 50)
